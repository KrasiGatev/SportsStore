...
namespace SportsStore.WebUI.CustomProviders
{
    public class RedisSessionStateStoreProvider : SessionStateStoreProviderBase
    {
        ...
        public override void Initialize(string name, System.Collections.Specialized.NameValueCollection config)
        {
            ...
            _host = string.IsNullOrEmpty(config["host"]) ? "nidoking.ec2.myredis.com" : config["host"];
            _port = string.IsNullOrEmpty(config["port"]) ? 9076 : int.Parse(config["port"]);
            _redisDb = string.IsNullOrEmpty(config["db"]) ? 0 : int.Parse(config["db"]);
            _password = string.IsNullOrEmpty(config["password"]) ? "uyySwEAA5BAMwT1y7wqq" : config["password"];
